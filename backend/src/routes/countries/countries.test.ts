...
describe("Continents-routes-testing", () => {
    ...
    const it_countries_response = [
        ...
        a.fk_continent_id === b.fk_continent_id ?
          a.it_name > b.it_name ? 1 : -1 :
          a.fk_continent_id - b.fk_continent_id
      );
    ...
    describe("IT language", () => {
        ...
        it("Gets the /list_single/:country_id endpoint", async () => {
            ...
            expect(obj[0]).toEqual(it_countries_response.filter(x => x.id === 5)[0]);
        });
        it("Gets the /list_single_by_iso_code/:iso_code endpoint", async () => {
            ...
            expect(response.status).toBe(200);
            const obj = JSON.parse(response.text);
            expect(obj[0]).toEqual(it_countries_response.filter(x => x.iso_alpha_3 === "ITA")[0]);
        });
        ...
        it("Gets the /countries_in_continents endpoint with a continent_id", async () => {
            ...
            const obj = JSON.parse(response.text);
            for(let i = 0; i < obj.length; i++) {
                expect(obj[i]).toEqual(it_countries_response.filter(x => x.fk_continent_id === 0)[i]);
            }
        });
        it("Gets the /countries_in_continents endpoint with 2 continents_id", async () => {
            ...
            const obj = JSON.parse(response.text);
            for(let i = 0; i < obj.length; i++) {
                expect(obj[i]).toEqual(it_countries_response.filter(x => x.fk_continent_id === 0 || x.fk_continent_id === 2)[i]);
            }
        });
        it("Gets the /country_of_city endpoint with no id", async () => {
            const response = await request.get(`${base_url}/country_of_city`);
            expect(response.status).toBe(400);
            const obj = JSON.parse(response.text);
            expect(obj).toEqual({error: error_codes.no_city_id});
        });
        it("Gets the /country_of_city endpoint with an id not present", async () => {
            const response = await request.get(`${base_url}/country_of_city?city_id=0`);
            expect(response.status).toBe(400);
            const obj = JSON.parse(response.text);
            expect(obj).toEqual([]);
        });
        it("Gets the /country_of_city endpoint with a present id", async () => {
            const response = await request.get(`${base_url}/country_of_city?city_id=1`);
            expect(response.status).toBe(200);
            const obj = JSON.parse(response.text);
            expect(obj[0]).toEqual(it_countries_response.filter(x => x.id === 5)[0]);
        });
